...
#[cfg(feature = "server")]
use crate::server::download::core::types::DownloadProgress;
#[cfg(feature = "server")]
use crate::server::download::core::utils;
#[cfg(feature = "server")]
use crate::server::download::storage::*;
...
pub async fn download_youtube_video(
    ...
    #[cfg(feature = "server")]
    {
        ...
        match download_with_quality(url.clone(), format_type.to_string(), quality.to_string()).await
        {
            Ok(_) => {
...
pub async fn download_with_quality(
    ...
    #[cfg(feature = "server")]
    {
        ...
        if let Some(download_dir) = dirs::download_dir() {
            ...
            if save_file_with_permissions(&download_path, &content) {
                tracing::info!(
...
pub async fn download_video_with_progress(
    ...
    // Use a custom progress tracker that updates both our local progress tracking
    ...
    match result {
        Ok(content) => {
            ...
            // Save to a permanent location with proper permissions
            let file_name = create_clean_filename(&title, "mp4");
            let mut saved_to_permanent = false;
