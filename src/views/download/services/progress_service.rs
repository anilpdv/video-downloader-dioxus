...
// Track download progress in a separate task
pub fn track_download_progress(
    url: String,
        ...
        async move {
            ...
            while download_in_progress_for_polling() {
                ...
                if seen_backend_complete && !download_complete {
                    ...
                    status_sig_for_polling.set(Some("Download complete! (100%)".to_string()));
                ...
                // Otherwise use time-based progress and update status when needed
                    ...
                    if new_time_progress > current_progress {
                        ...
                        #[cfg(not(feature = "web"))]
                        let milestone_elapsed_secs = last_status_update.elapsed().as_secs() as f64;
                            ...
                            && milestone_elapsed_secs >= 2.0
                        {
                            ...
                            status_sig_for_polling.set(Some(
                                milestone_message.replace("{}", &current_progress.to_string()),
                            ));
                    ...
                    #[cfg(feature = "web")]
                    let stuck_elapsed_secs = (js_sys::Date::now() - last_status_update_ms) / 1000.0;
                    ...
                    if current_progress >= 85 && !seen_backend_complete && stuck_elapsed_secs >= 5.0
                    {
                        // Update status to indicate we're still working
                        status_sig_for_polling
                            .set(Some(format!("Processing video... ({}%)", current_progress)));
